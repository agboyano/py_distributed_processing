...
class Worker():
    def __init__(self, serializer, connector, worker_id=None, with_trace=True, reply_to_default=None):
        ...
        self.worker_id = worker_id if worker_id is not None else self.connector.get_server_id()
        logger.info(f"Worker id: {self.worker_id}")

    def add_requests_queue(self, simple_queue_name, func_dict, priority=10, register=True):
    ...
    def add_function(self, simple_queue_name, fn_name, fn, priority=10, register=True):
        ...
        if requests_queue not in self.requests_queues:
            self.add_requests_queue(simple_queue_name, {fn_name: fn}, priority, register)
        else:
    ...
    def add_python_eval(self, simple_queue_name="py_eval", priority=20, register=True):
        ...
        """
        self.add_function(simple_queue_name, "eval_py_function", eval_py_function, priority, register)
    ...
    def add_requests_queues(self, queues, register=True):
        ...
        for simple_queue_name in queues:
            self.add_requests_queue(simple_queue_name, queues[simple_queue_name][0], 
                                    priority=queues[simple_queue_name][1], register=register)
        # no olvidar self.update_registry()

    def update_methods_registry(self):
        queues_to_register = {k: v[0] for (k, v) in self.requests_queues.items()
    ...
    def enhance_response(self, response, request, dispatched_to=None, execution_start=None):
        r = response
        # To be cleaned
    ...
    def process_single_request(self, request, dispatched_to):
        ...
        if not is_single_request(request):
            logger.error(f"{timestamp()} Couldn't be processed Individual Request from Batch Request from queue: {dispatched_to}")
        ...
        # send ack message
        if not is_notification and reply_to is not None and "ack" in request and request["ack"]:
            msg = ack(id_, self.worker_id, dispatched_to)
            self.connector.enqueue(reply_to, self.serializer.dumps(msg))
            logger.debug(f"{timestamp()} Worker: {self.worker_id} Sent Ack to queue: {reply_to} for request {id_}")
    ...
    def process_request(self, request, dispatched_to):
        ...
        if is_single_request(request):
            logger.debug(f"{timestamp()} Worker: {self.worker_id} Received Single Request for method: {request['method']} from queue: {dispatched_to}")
            ...
            logger.debug(f"{timestamp()} Worker: {self.worker_id} Received Batch Request with {len(request)} requests from queue: {dispatched_to}")
            if len(request) == 0:
                return None # error_response(-32600)
                        
            responses = [self.process_single_request(rq, dispatched_to) for rq in request]
            ...
            # Should not happen
            logger.error(f"{timestamp()} Worker: {self.worker_id} No responses where processed for Batch Request from queue: {dispatched_to}")
            return None # error_response(-32600)

        else:
            logger.error(f"{timestamp()} Worker: {self.worker_id} Received neither a Single nor a Batch Request from queue: {dispatched_to}")
            return None # error_response(-32600)

    def shuffled_queues(self):
        tuples = [(k, v[1]) for k, v in self.requests_queues.items()] # [(queue, priority), ...]
        grouped = {}
        for k, v in tuples:
    ...
    def run_once(self, timeout=-1):
        ...
        if request_with_priority is not None:
            ...
            try:
                ...
                logger.error(f"{timestamp} Worker: {self.worker_id} Message from queue {dispatched_to} couldn't be deserialized.")
                return 
            ...
            if is_single_response(processed):
                ...
                is_notification = processed["is_notification"] or id_ is None or reply_to is None
                ...
                if not is_notification:
                    ...
                    logger.debug(f"{timestamp()} Worker: {self.worker_id} Sent Single {rtype} Response with id {id_} for method: {method} to queue: {reply_to}")
                else:
                    logger.debug(f"{timestamp()} Worker: {self.worker_id} Processed Notification for method: {method} from queue: {dispatched_to}")
                ...
                for one_single_response in processed:
                    ...
                    is_notification =  one_single_response["is_notification"] or id_ is None or reply_to is None
                    rtype = "RESULT" if "result" in one_single_response else "ERROR"
                    if not is_notification:
                        logger.debug(f"{timestamp()} Worker: {self.worker_id} Appending {rtype} response with id: {id_} for method: {method} from Batch Request from queue: {dispatched_to}")
                        self.clean_response(processed)
                        batch.setdefault(reply_to, []).append(one_single_response)
                    else:
                        logger.debug(f"{timestamp()} Worker: {self.worker_id} Processed Notification for method: {method} from Batch Request from queue: {dispatched_to}")
                
                for reply_queue in batch:
                    self.connector.enqueue(reply_queue, self.serializer.dumps(batch[reply_queue]))
                    logger.debug(f"{timestamp()} Worker: {self.worker_id} Sent Batch Response with {len(batch[reply_queue])} items to queue {reply_queue}")
        else:
            logger.debug(f"{timestamp()} Worker: {self.worker_id} run_once method timeout")
